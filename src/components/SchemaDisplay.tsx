...
import { Database, Hash, Calendar, ToggleLeft, Tag, FileText, MapPin } from 'lucide-react';
        ...
        {schema.columns.map((col, index) => (
          ...
          >
            <div className={cn(
              ...
              {typeIcons[col.type]}
              <span className="text-xs font-medium uppercase">{col.type}</span>
            </div>

            <div className="flex-1 min-w-0">
              <p className="font-mono text-sm text-foreground truncate">{col.name}</p>
            </div>
